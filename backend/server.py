...
def build_transcript_pdf(request: dict) -> io.BytesIO:
    ...
    school_para = Paragraph(
        ...
        "Guidance Department: 876 922 8254 &nbsp;|&nbsp; Email: wbs.guidance@wolmers.org",
        ParagraphStyle('School', parent=styles['Normal'], fontSize=9, textColor=dark_text, leading=14, alignment=1)
    )
    ...
    story.append(Paragraph(f"Status: <b>{status}</b>  |  Submitted: <b>{format_date_for_export(request.get('created_at',''))}</b>  |  Needed By: <b>{format_date_for_export(request.get('needed_by_date',''))}</b>", subtitle_style))
    ...
    def two_col_row(l1, v1, l2="", v2=""):
        return [
            [Paragraph(l1, label_style), Paragraph(v1 or "—", value_style),
             Paragraph(l2, label_style), Paragraph(v2 or "—", value_style)]
        ]
    ...
    rows += two_col_row("School ID", request.get('school_id') or "—", "Enrollment Status", (request.get('enrollment_status') or "").replace('graduate','Graduate/Alumni').title())
    ...
    rows += two_col_row("Academic Years", years_str, "Last Form Class", request.get('last_form_class'))
    t = Table(rows, colWidths=[95, 160, 95, 120])
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, -1), HexColor('#fafaf9')),
        ('ROWBACKGROUNDS', (0, 0), (-1, -1), [HexColor('#fafaf9'), HexColor('#f5f5f4')]),
    ...
    rows2 = []
    rows2 += two_col_row("Wolmer's Email", request.get('wolmers_email') or request.get('student_email'), "Personal Email", request.get('personal_email'))
    rows2 += two_col_row("Phone Number", request.get('phone_number'), "School ID", request.get('school_id') or "—")
    t2 = Table(rows2, colWidths=[95, 160, 95, 120])
    ...
    exams_str = format_external_exams(request.get('external_exams', []))
    story.append(Paragraph(exams_str, value_style))
    story.append(Spacer(1, 8))
    ...
    rows3 = []
    rows3 += two_col_row("Reason", request.get('reason'), "Number of Copies", str(request.get('number_of_copies') or 1))
    rows3 += two_col_row("Collection Method", coll_map.get(request.get('collection_method',''), request.get('collection_method','')), "Received Before?", request.get('received_transcript_before') or "—")
    ...
    t3 = Table(rows3, colWidths=[95, 160, 95, 120])
    ...
    rows4 = []
    rows4 += two_col_row("Institution Name", request.get('institution_name'), "Phone", request.get('institution_phone'))
    rows4 += two_col_row("Email", request.get('institution_email'), "", "")
    rows4 += [[Paragraph("Address", label_style), Paragraph(request.get('institution_address',''), value_style), '', '']]
    t4 = Table(rows4, colWidths=[95, 160, 95, 120])
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph("Administrative Clearance", ParagraphStyle('ClearHeader', parent=styles['Normal'], fontSize=12, textColor=maroon, fontName='Helvetica-Bold', spaceBefore=4, spaceAfter=6)))
    ...
    def checkbox_row(label: str, checked: bool) -> list:
        ...
        d = Drawing(12, 12)
        d.add(Rect(0, 0, 12, 12, strokeColor=HexColor('#7b1e2c'), strokeWidth=1, fillColor=HexColor('#ffffff')))
        if checked:
            d.add(Line(2, 6, 5, 2, strokeColor=HexColor('#7b1e2c'), strokeWidth=1.5))
            d.add(Line(5, 2, 11, 10, strokeColor=HexColor('#7b1e2c'), strokeWidth=1.5))
        return [d, Paragraph(f"<b>{label}</b>", check_style)]
    ...
    clearance_table_data = [
        ...
        [Paragraph(f"<b>Amount Paid:</b> {ac['amount']}", value_style), Paragraph(f"<b>Receipt #:</b> {ac['receipt']}     <b>Date:</b> {ac['date']}", value_style)],
    ]
    if ac['updated_by'] != '—':
        clearance_table_data.append([Paragraph(f"<b>Processed by:</b> {ac['updated_by']}", small_style), ''])

    ct = Table(clearance_table_data, colWidths=[20, 450])
    ct.setStyle(TableStyle([
        ...
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ('SPAN', (0, 2), (0, 2)),
        ('SPAN', (0, 3), (0, 3)) if ac['updated_by'] != '—' else ('SPAN', (0, 0), (0, 0)),
    ]))
    story.append(ct)
    story.append(Spacer(1, 8))

    if request.get('staff_notes'):
        story.append(Paragraph(f"<b>Staff Notes:</b> {request['staff_notes']}", small_style))
...
def build_transcript_docx(request: dict) -> io.BytesIO:
    ...
    school_lines = [
        ...
        ("Guidance Department: 876 922 8254  |  Email: wbs.guidance@wolmers.org", False, Pt(9), grey_rgb),
    ]
    first = True
    ...
    def add_field_table(rows_data):
        """rows_data: list of (label, value, label, value) or (label, value)"""
        cols = 4
        ...
        for i, row_data in enumerate(rows_data):
            ...
            if len(row_data) == 4:
                ...
                # merge cols 1,2,3 for single value
                row.cells[0].text = ""
                row.cells[1].text = ""
                row.cells[0].paragraphs[0].runs and None
                row.cells[0].paragraphs[0].add_run(str(row_data[0])).font.color.rgb = grey_rgb
                row.cells[0].paragraphs[0].runs[-1].font.size = Pt(9)
                merged = row.cells[1].merge(row.cells[2]).merge(row.cells[3])
                r = merged.paragraphs[0].add_run(str(row_data[1]) if row_data[1] else "—")
                r.bold = True
                r.font.size = Pt(9)
    ...
    add_field_table([
        ...
        ("School ID", request.get('school_id') or "—", "Enrollment Status", status_disp),
    ...
    add_field_table([
        ("Wolmer's Email", request.get('wolmers_email') or request.get('student_email',''), "Personal Email", request.get('personal_email','')),
    ...
    coll_map = {'pickup': 'Pickup at Bursary', 'emailed': 'Email to Institution', 'delivery': 'Courier Delivery (DHL)'}
    rows_req = [
        ("Reason", request.get('reason',''), "Number of Copies", str(request.get('number_of_copies') or 1)),
        ("Collection Method", coll_map.get(request.get('collection_method',''), ''), "Received Before?", request.get('received_transcript_before') or "—"),
    ]
    if request.get('other_reason'):
        rows_req.append(("Other Reason", request.get('other_reason',''), "", ""))
    if request.get('delivery_address'):
        rows_req.append(("Delivery Address", request.get('delivery_address',''), "", ""))
    add_field_table(rows_req)
    ...
    add_field_table([
        ...
        ("Email", request.get('institution_email',''), "", ""),
    ...
    def set_check_cell(cell, label, checked):
        p = cell.paragraphs[0]
        if checked:
            run_check = p.add_run('\u2611 ')  # ☑ - will use fallback
            try:
...
def build_recommendation_pdf(request: dict) -> io.BytesIO:
    ...
    def checkbox_row_gold(label: str, checked: bool) -> list:
        from reportlab.graphics.shapes import Drawing, Rect, Line
        d = Drawing(12, 12)
        d.add(Rect(0, 0, 12, 12, strokeColor=HexColor('#b8860b'), strokeWidth=1, fillColor=HexColor('#ffffff')))
        if checked:
            d.add(Line(2, 6, 5, 2, strokeColor=HexColor('#b8860b'), strokeWidth=1.5))
            d.add(Line(5, 2, 11, 10, strokeColor=HexColor('#b8860b'), strokeWidth=1.5))
        return [d, Paragraph(f"<b>{label}</b>", check_style)]
    ...
    ct = Table(clearance_data, colWidths=[20, 450])
    ct.setStyle(TableStyle([
